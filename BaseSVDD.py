...
class BaseSVDD(BaseEstimator, OutlierMixin):
    ...
    def solve_problem(self, K):
        ...
        self.support_vector_indices = np.where(
            np.abs(self.alpha) > self.alpha_tolerance)[0][:]
        ...
        self.boundary_indices = self.support_vector_indices[np.array(
            list(set(tmp_3) & set(tmp_4)))]

        # support vectors
        self.alpha[np.where(np.abs(self.alpha) <
                            self.alpha_tolerance)[0][:]] = 0
        ...
        self.radius = np.sqrt(
            np.mean(np.diag(K)) + self.offset+np.mean(tmp_[self.boundary_indices, 0]))
    ...
    def predict(self, X, y=None):
        ...
        start_time = time.time()
        results = {}
        results['X'], results['y'], results['y_type'], results['exist_y'] = self._check_X_y(
            X, y)
    ...
    def display_fit(self):
        ...
        print('ratio of SVs         = %.4f %%' %
              (100*self.n_support_vectors_ratio))
    ...
    def plot_boundary(self, X, y=None, expand_ratio=0.2, n_grids=50,
        ...
        if dim != 2:
            raise SyntaxError(
                'Visualization of decision boundary only supports for 2D data')
        ...
        ax1.contourf(xv, yv, distance.A, n_level, zdir='z',
                     offset=np.min(distance)*0.9, cmap=color_map)
        ax1.set_zlim(np.min(distance)*0.9, np.max(distance)*1.05)

        # figure 2: the 2D contour
        ax2 = fig.add_subplot(1, 3, 2)
        ctf1 = ax2.contourf(xv, yv, distance, n_level,
                            alpha=0.8, cmap=color_map)
        ctf2 = ax2.contour(xv, yv, distance, n_level,
...
class BananaDataset():
    ...
    def generate(**kwargs):
        ...
        data_p = np.append(sizeBanana * np.sin(class_p),
                           sizeBanana * np.cos(class_p), axis=1)
        ...
        data_n = np.append(sizeBanana * np.sin(class_n),
                           sizeBanana * np.cos(class_n), axis=1)
        data_n = data_n + np.random.rand(number, 2)*varBanana
        data_n = data_n + np.ones((number, 1)) * \
            [sizeBanana * param_4, sizeBanana * param_4]
    ...
    def split(data, label, **kwargs):
        ...
        pIndex = y_train == 1
        nIndex = y_train == -1
        X_train = np.append(X_train[pIndex[:, 0], :],
                            X_train[nIndex[:, 0], :], axis=0)
        y_train = np.append(y_train[pIndex[:, 0], :],
                            y_train[nIndex[:, 0], :], axis=0)

        pIndex = y_test == 1
        nIndex = y_test == -1
        X_test = np.append(X_test[pIndex[:, 0], :],
                           X_test[nIndex[:, 0], :], axis=0)
        y_test = np.append(y_test[pIndex[:, 0], :],
                           y_test[nIndex[:, 0], :], axis=0)
